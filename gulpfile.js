...
gulp.task('tsc', function () {
  ...
  return merge(
    ...
    tsResult.js.pipe(gulp.dest('.')).pipe(
      through.obj(function (chunk, enc, cb) {
        if (typescript_error_count) {
          this.emit(
            'error',
            'TypeScript compile errors (count:' + typescript_error_count + ')'
          );
        }
        cb(null, chunk);
      })
...
gulp.task(
  'assemble',
  gulp.series(['tsc'], function () {
    var config = require('./rollup.config');

    return rollup.rollup(config).then(function (bundle) {
      return Promise.all(config.output.map(function (_) {
        return bundle.write(_)
      }));
    });
  })
);
...
gulp.task('prepare_test', function () {
  return gulp.src(libs_for_test).pipe(gulpCopy('test/libs/', { prefix: 2 }));
});

gulp.task('compress', function (cb) {
  return gulp
    .src('bin/jsencrypt.js')
    .pipe(terser())
...
gulp.task(
  'build',
  gulp.series(['prepare_test', 'lint', 'assemble', 'license', 'compress'])
);
